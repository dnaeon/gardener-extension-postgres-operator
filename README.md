...
For local development of the `gardener-extension-postgres-operator` it is recommended that
...
``` shell
$ kubectl get controllerregistrations,controllerdeployments gardener-extension-postgres-operator
NAME                                                                    RESOURCES           AGE
controllerregistration.core.gardener.cloud/gardener-extension-postgres-operator   Extension/postgres   40s

NAME                                                                  AGE
controllerdeployment.core.gardener.cloud/gardener-extension-postgres-operator   40s
```
...
`gardener-extension-postgres-operator`, since it is required by our shoot.
...
gardener-extension-postgres-operator-tktwt   gardener-extension-postgres-operator   local   True    True        True      False         103s
```
...
$ kubectl --namespace shoot--local--local get extensions
NAME      TYPE      STATUS      AGE
postgres   postgres   Succeeded   85m
```
...
``` shell
kubectl --namespace shoot--local--local annotate extensions postgres gardener.cloud/operation=reconcile
```
...
``` shell
$ kubectl --kubeconfig $KUBECONFIG_RUNTIME get extop gardener-extension-postgres-operator
NAME                         INSTALLED   REQUIRED RUNTIME   REQUIRED VIRTUAL   AGE
gardener-extension-postgres-operator   True        False              False              85s
```
...
NAME                                                                  AGE
controllerdeployment.core.gardener.cloud/gardener-extension-postgres-operator   3m50s
```
...
NAME                               REGISTRATION                 SEED    VALID   INSTALLED   HEALTHY   PROGRESSING   AGE
gardener-extension-postgres-operator-ng4r8   gardener-extension-postgres-operator   local   True    True        True      False         2m9s
```
...
NAME      TYPE      STATUS      AGE
postgres   postgres   Succeeded   2m37s
```
...
``` shell
kubectl --kubeconfig $KUBECONFIG_RUNTIME --namespace shoot--local--local annotate extensions postgres gardener.cloud/operation=reconcile
```
...
`gardener-extension-postgres-operator` is hosted on
[Github](https://github.com/gardener/gardener-extension-postgres-operator).
